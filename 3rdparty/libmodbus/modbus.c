    ...
    case _FC_REPORT_SLAVE_ID: {
      //  int str_len;
       ...
       // rsp[rsp_length++] = _REPORT_SLAVE_ID;
        ...
        rsp[rsp_length++] = ctx -> AppStatus;
        rsp[rsp_length++] = ctx -> AppHardVersion;
        rsp[rsp_length++] = ctx -> AppSoftVersion;
...
void _modbus_init_common(modbus_t *ctx)
{
    ...
    ctx -> AppHardVersion = -1;
    ctx -> AppSoftVersion = -1;
    ctx -> AppStatus = 0x00;
}
...
void modbus_set_app_softversion(modbus_t *ctx, unsigned char ver)
{
    ctx -> AppSoftVersion = ver;
}


void modbus_set_app_hardversion(modbus_t *ctx, unsigned char ver)
{
    ctx -> AppHardVersion = ver;
}


void modbus_set_app_status(modbus_t *ctx, int status)
{
    if (status) ctx -> AppStatus = 0xFF; else ctx -> AppStatus = 0x00;
}
